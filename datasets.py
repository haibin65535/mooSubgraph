...
def get_mnistsp(args):
    ...
    train_val = MNIST75sp(args.data_root + 'data/MNISTSP/', mode='train')
    ...
    test_dataset = MNIST75sp(args.data_root + 'data/MNISTSP/', mode='test')
...
def get_molhiv(args):
    ...
    dataset = PygGraphPropPredDataset(name = 'ogbg-molhiv', root=args.data_root + 'data') 
    if not hasattr(dataset, 'uny'):
        datalist  = []
        num_class = dataset.num_classes
        dataset.data.x = dataset.data.x.float()
        for i in range(len(dataset)): 
            data = dataset[i]   
            data.uny = 1 - F.one_hot(data.y, num_class).float() 
            data.split_n = data.x.shape[0]
            data.split_e = data.edge_index.shape[1]
            datalist.append(data)
        torch.save(dataset.collate(datalist), dataset.processed_paths[0])
        dataset = PygGraphPropPredDataset(name = 'ogbg-molhiv', root=args.data_root + 'data') 
...
def get_SPMotif(args):
    ...
    traing_dataset = SPMotif(args.data_root + 'data/'+ f'SPMotif-{args.bias}/', mode='train')
    val_dataset = SPMotif(args.data_root + 'data/'+ f'SPMotif-{args.bias}/', mode='val')
    test_dataset = SPMotif(args.data_root + 'data/'+ f'SPMotif-{args.bias}/', mode='test')
...
def get_sst(args):
    ...
    dataset = get_dataset(dataset_dir=args.data_root + 'data/', dataset_name='Graph_SST2', task=None)
